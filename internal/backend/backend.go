...
func LoadConfiguration(dirs []string, chdir string, vars variable.Variables) (backend map[string]string, err error) {
...
func varsEvalContext(vars variable.Variables) *hcl.EvalContext {
	ctx := hcl.EvalContext{}
	values := map[string]cty.Value{}
	for _, v := range vars {
